...
export function CreateMatchModal({ chain, onClose, onSuccess }: Props) {
  ...
  async function handleCreate() {
    ...
    if (privyConfigured && (!ready || !authenticated)) {
      try {
        login()
      } catch (e) {
        console.warn('Privy login not available, continuing in demo mode')
      }
      return
    }
  ...
  const handleDone = () => {
    // Set the created match as active and navigate to game view
    if (createdMatch) {
      const { setActiveMatch, setCurrentView } = useGameStore.getState()
      setActiveMatch(createdMatch)
      setCurrentView('game')
    }
    onSuccess()
    onClose()
